"""Template for driving the card in the dev HA with Playwright (see ai/local-ha-testing.md).
...
async def scenario(page: Page, card, cdp) -> None:
    ...
    await tap(page, card.locator(".header ha-icon-button:has(ha-icon[icon='mdi:pencil'])").first)
    ...
    async with async_playwright() as p:
        ...
        await page.add_init_script(
            """localStorage.setItem('hassTokens', JSON.stringify({
                access_token: '%s', token_type: 'Bearer', expires_in: 1800,
